...
io.on('connection', socket=>{
  ...
  socket.on('spinStarter',({roomCode})=>{ const rc=cleanCode(roomCode); const room=rooms[rc]; if(!room) return socket.emit('errorMessage','Room not found.'); const seatedNow=seated(room); if(seatedNow.length<2) return socket.emit('errorMessage','Need at least 2 seated players or bots.'); const win=seatedNow[Math.floor(Math.random()*seatedNow.length)]; room.starterToken=win.token; emitRoom(rc); io.to(rc).emit('starterChosen',{token:win.token,id:win.id,name:win.name}); });
